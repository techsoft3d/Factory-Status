...
robotViewer.prototype._createViewer = function () {
    var _this = this;
    createViewer("factory", "container").then(function (hwv) {
        _this._viewer = hwv;
        ...
        function modelStructureReadyFunc() {
            ...
            _this._createRobotSystem("Arc Welding #1", "WeldRobot1", "csv/WeldRobotMatrixData.csv", { x: 1500, y: -1500, z: 0 }).then(function () {
                ...
                $.getJSON("json/operating_ratio.json", function (ratioData) {
                    if (ratioData) {
                        _this._ratios = [];
                ...
                setInterval(function() {
                    $.getJSON('https://factorystatusbackend_linux.techsoft3d.com/demos/factory-status/json/status.json', (data) => {
                        ...
                        for (var i = 0; i < _this._robotSystems.length; i++) {
                            if (data[String(i)]) {
                                ...
                                } else if (obj.status == "stop" || obj.status == "stop" && currentStatus == "run") {
